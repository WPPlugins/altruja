...
class AltrujaAdmin {
  ...
  public function validate($options) {
    ...
    if (is_wp_error($response = wp_remote_get('http://www.altruja.de/api/integration/email/'.urlencode($email), array('redirection' => 10)))) {
      return array();
    }
  ...
  public function section () {
    echo strtr(__('Please provide the email address you use to login to [my]My Altruja[/my], or [reg]register an account[/reg].', 'altruja'), array(
      '[my]' => '<a href="http://altruja.de/myaltruja">',
      '[/my]' => '</a>',
      '[reg]' => '<a href="http://altruja.de/register">',
      '[/reg]' => '</a>',
    ));
  }
